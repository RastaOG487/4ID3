...
void loop() {
  ...
  client.publish("4ID3_Larry/temperature", String(dhtTempEvent.temperature).c_str());
  delay(100);
  client.publish("4ID3_Larry/humidity", String(dhtHumEvent.relative_humidity).c_str());
  delay(100);
  client.publish("4ID3_Larry/pressure", String(bmpEvent.pressure).c_str());
  delay(100);
  client.publish("4ID3_Larry/light", String(lux).c_str());
